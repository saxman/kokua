...
can call `compose_worker` to build a sub-agent from anything installed, chosen per task rather than
declared: the exception is entered by declaration, and what it grants dies with the task rather than
widening any agent your config describes.
