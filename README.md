...
Kokua needs Python 3.11+ and [AIMU](https://github.com/saxman/aimu) 0.29.0 or newer.
...
> ```
>
> The `aimu>=0.29.0` requirement governs the PyPI path only: uv installs a path source without checking it against the specifier, so a sibling checkout is not constrained by it. If yours falls behind, startup says so and names the fix rather than failing on an import.
