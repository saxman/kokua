...
Useful flags: `--list-toolsets` (every capability name this install accepts, grouped by what provides it), `--mcp <url>` (repeatable), `--no-plugins`, `--system`, `--config <path>`, `--host` / `--port` (web). What each agent can *do* is not a flag: it is that agent's `tools` list in `config.toml` (see [Configuration](#configuration)).
