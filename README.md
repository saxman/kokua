...
- **It can analyze how another conversation ran, not just what it said.** The three reading tools drop reasoning, tool calls, and tool output on purpose. `export_conversation` keeps all of it: ask why a turn failed, which tool returned what, or what a run cost, and the assistant renders that conversation to the same Markdown `kokua export` writes and answers from it. It hands back a *path*, not the transcript, because a run's full tool output would swallow the context of the conversation asking about it: a long export goes to the shipped `analyst` sub-agent, whose own fresh context absorbs the file and returns only the findings. You get the file too, in `~/.kokua/data/downloads/`, so you can check the answer against the evidence it came from.
