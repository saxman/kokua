...
[![CI](https://github.com/saxman/kokua/actions/workflows/ci.yml/badge.svg)](https://github.com/saxman/kokua/actions/workflows/ci.yml) ![GitHub License](https://img.shields.io/github/license/saxman/kokua) ![Python Version from PEP 621 TOML](https://img.shields.io/python/required-version-toml?tomlFilePath=https%3A%2F%2Fraw.githubusercontent.com%2Fsaxman%2Fkokua%2Frefs%2Fheads%2Fmain%2Fpyproject.toml) [![uv](https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/astral-sh/uv/main/assets/badge/v0.json)](https://github.com/astral-sh/uv) [![Ruff](https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/astral-sh/ruff/main/assets/badge/v2.json)](https://github.com/astral-sh/ruff) [![Docs](https://img.shields.io/badge/docs-saxman.info%2Fkokua-4f46e5)](https://saxman.info/kokua/)

[Documentation](https://saxman.info/kokua/) · [Design principles](docs/explanation/design-principles.md) · [Architecture](docs/explanation/architecture.md) · [Configuration](docs/reference/configuration.md) · [Contributing](CONTRIBUTING.md) · [Changelog](CHANGELOG.md)

</div>

**Kokua** (Hawaiian: *help, assistance*) is a real, always-on personal assistant, designed to help people understand how agentic assistants work. It is hackable and modular (OpenClaw / Hermes Agent style), built on the [AIMU](https://saxman.info/aimu/) library: it chats with you, authors and runs its own skills, connects to remote tool services, delegates independent subtasks to isolated sub-agents, schedules its own proactive work, and remembers facts and documents across conversations. None of that is a demo, and every one of those mechanisms is built to be read, run, and extended.
...
Kokua is a real assistant built to be understood. It does real work, because a toy cannot teach what real work costs, and its machinery is meant to be followed rather than taken on faith: that combination is what lets you learn how an agentic assistant works from the one you are actually using. There are three ways in.
...
| [core/turns.py](src/kokua/core/turns.py) | What one turn is, reactive and proactive. It opens with eight concurrency invariants, each naming the bug it prevents. |
...
None of these is a mode or a preset. Each is the same declared machinery pointed at a different job, and each one says what you have to add yourself, because a capability Kokua did not ship with arrives through the same seam its own do.

**A market watch that has to check its numbers.** *"Every weekday at 4:30, check my positions, flag anything that moved more than 3%, and email me the digest."* The assistant writes that as a [`[scheduling.task.*]` table](#acting-without-you) in `config.toml` through its own `schedule_task`, so it survives restarts and you can read and edit it by hand. Each firing gets its own conversation and can fan out to concurrent `researcher` [sub-agents](#agents-and-delegation), one per ticker, each with a context built fresh for the lookup. A price is exactly the kind of fact the `web` toolset's guidance says to go and check ["even when you think you know"](#memory-and-context); `compute` does the arithmetic; `documents` and `memory` carry the positions and the thesis from one firing to the next; the [`email-report` skill](skills/email-report/) delivers the digest. Turn on [adversarial review](#planning-and-self-review) and a reviewer with no conversation context verifies the recency and numeric claims before you read them. The constraint is worth watching as much as the capability: a proactive turn auto-denies every approval-gated tool, so what a scheduled run reaches is exactly what you granted it and nothing it can talk its way into. **You add:** the skill, an agent that declares `web`, and your own judgment about the result, which is sourced and checkable in the trace rather than advice.

**A job hunt, with the expensive step in someone else's package.** *"Here is the posting. Tailor my resume and draft the cover letter."* [jobme](https://github.com/saxman/jobme) is a separate project that does exactly that, and it is the first package outside this repository to register a Kokua toolset: one `kokua.toolsets` entry point, the same seam Kokua's own 22 arrive through, and no change to Kokua's core. The name `jobme` lands in the namespace and reaches nothing until an agent's `tools` list names it; its `[jobme]` section in `config.toml` is seeded from `Setting` declarations on the toolset itself; and because `tailor_application` spends several minutes and real provider money per call, it belongs in [`[security] confirm_tools`](#security), where you approve each run. It also carries its own `job-application` skill, teaching the model the procedure around the tool. **You add:** the package, one name in an agent's `tools`, and the gate. [Install a third-party toolset](docs/how-to/install-a-third-party-toolset.md) walks all four steps with jobme as the worked example.

**A machine that looks after itself.** *"Check disk and the log directory every morning, and back yourself up nightly."* This is the [security model](#security) made concrete instead of described. Reading a file and writing one are two toolsets, `fs` and `fs_write`, because with a single name there would be no way to declare "read a file and do not write one". `run_command` and every writer in `fs_write` are gated per call, a `y/N` in the terminal or Allow/Deny in the browser. Switch on `[security.auto_approval]` and a declared reviewer can answer that prompt in your place on a call it can justify, with no deny verdict, so it can never widen what you could have approved yourself. And `github_backup`'s one tool takes no arguments at all, deliberately: that is what lets it stay ungated and therefore run unattended on a schedule, where jobme's tool above cannot. **You add:** a private repository and a token ([back up to GitHub](docs/how-to/back-up-to-github.md)); the rest ships.

**A reading desk that remembers.** *"Read these six papers and tell me where they disagree." "What did we decide about this last month?"* The [document store](#memory-and-context) is a folder you drop files into, semantic memory is facts that outlive the conversation, and both are shared by every agent declaring them. The [`synthesize-documents` skill](skills/synthesize-documents/) exists because the obvious path, search and then summarize the hits, produces a fluent answer built from first pages, and nothing about that answer looks wrong. `search_conversations` makes a thread from last month an input to this one, and when the question is about how a run went rather than what it said, `export_conversation` hands back a *path*, so the whole transcript lands in the `introspector` sub-agent's fresh context instead of swallowing yours. Context is a budget, and these are the tools that make the budget visible. **You add:** `kokua skills install synthesize-documents`.
...
Five commands worth knowing. **`/new`** starts a fresh conversation, **`/conversations`** lists every one you have with a short id and a `*` on the one you are in, and **`/switch <id>`** moves to another (the web UI's sidebar does all three with the mouse; typing them works there too). Switching never cancels a reply already running: it keeps going and saves to the conversation it started in, which the reply to your command says out loud when it applies. **`/stop`** cancels a reply that's still streaming and keeps the partial turn, so the conversation continues (the web UI has a Stop button for the same). **`/diag`** reports the in-flight turn, the gate state, and a stuck turn's async stack; it never takes the turn gate, so it answers even when a hung turn is holding it. (In the web UI it does wait behind an earlier control that is itself waiting on the gate, because the page's frames are applied in the order they arrive; reloading clears that, and always can.) Rotating logs go to `$KOKUA_HOME/data/logs/kokua.log`, and `kill -USR1 <pid>` dumps all thread stacks there.
...
- **A reactive turn runs as a background handle.** [`core/turns.py`](src/kokua/core/turns.py) opens with eight concurrency invariants, each naming the bug it prevents. The turn runs as an AIMU `RunHandle` while the channel keeps reading, and that one decision is what makes the rest work: **`/stop`** can cancel a reply mid-stream and keep the partial turn, and a web Allow/Deny click can reach the tool call still waiting on it.
- **Switching conversations does not cancel a turn.** Each conversation owns its own agent and model client, so a reply you walk away from keeps running: it persists to its own conversation, mutes its stream, and raises a dismissible alert card when it lands. Switch back into one still working and it catches you up (the reasoning, tool calls, and answer so far, with a spinner counting off how long that turn has been going and Stop in the composer) then keeps streaming from where it left off. That is the web UI; in the terminal `/new` and `/switch` background a turn the same way, and since a terminal has no second place to put it, the reply keeps printing where you are now, which the command's own answer warns you about.
...
- **[Export a conversation to Markdown](docs/how-to/export-a-conversation.md).** `kokua export` writes one saved conversation as a Markdown file you can diff, paste into a review, or keep once the terminal is gone: every turn's reasoning, tool calls with their arguments and results, and what it cost. It reads the session store directly and builds no assistant, so it works with the model server down. `kokua export <id-or-prefix> -o -` writes to stdout instead of a file; `--full` lifts the truncation a plain export applies to long tool payloads. The web UI's sidebar has the same export as a download button beside each conversation's delete button, so you don't need the command line to get the file. The assistant holds the same export as `export_conversation`, which is how you ask it a question *about* a run instead of reading the file yourself.
...
- **[Every agent is declared, none is baked in](docs/how-to/set-up-toolsets.md).** One `[agents.<name>]` table per agent in `config.toml`, each with a `description`, a `system_message`, an optional `model`, `thinking`, and `max_iterations`, a `tools` list of toolset names, and a `delegates_to` list. `[assistant].model`, `[assistant].thinking`, and `[assistant].max_iterations` are the model every agent runs on, the reasoning effort it runs at, and the most model calls its turn may make (default 10); an agent naming its own runs on that instead, so a lean supervisor and an expensive researcher need not share any of the three. Resolution is per agent and never inherited, so a delegator that pins a model, reasons hard, or raises its own cap does not drag its workers along. `thinking` takes `false` (do not reason), `true` (reason at the model's own default), or `"low"` / `"medium"` / `"high"`; unset, nothing is sent and each model keeps its own behavior. `[assistant].agent` says which one you talk to. `kokua config init` writes `assistant`, `researcher`, `coder`, and `introspector`, and you edit, delete, or add to them freely. At least one agent is required, so a config with none is refused at startup rather than run as an assistant that can do nothing.
...
- **It can read its own history.** `list_conversations`, `read_conversation`, and `search_conversations` give the assistant sight of every saved conversation, so it can answer "what did we decide about this last week?" and carry context out of a past thread into a sub-agent's task. A transcript comes back as bounded plain text (what was said, no reasoning or tool calls), oldest messages dropped first behind a count when it does not fit, which is the context window made visible as a rule you can read. Reads come from the last saved snapshot, so a conversation still streaming is flagged as having unsaved messages, and the one you are in is flagged too, since its current turn is not saved yet.
- **It can analyze how another conversation ran, not just what it said.** The three reading tools drop reasoning, tool calls, and tool output on purpose. `export_conversation` keeps all of it: ask why a turn failed, which tool returned what, or what a run cost, and the assistant renders that conversation to the same Markdown `kokua export` writes and answers from it. It hands back a *path*, not the transcript, because a run's full tool output would swallow the context of the conversation asking about it: a long export goes to the shipped `introspector` sub-agent, whose own fresh context absorbs the file and returns only the findings. You get the file too, in `~/.kokua/data/downloads/`, so you can check the answer against the evidence it came from. That sub-agent, `introspector`, is the one shipped worker that reads history, because it is the one with a conversation as its subject: ask it to evaluate a past run against criteria you name and it finds the conversation, exports it, and reports per criterion with the transcript lines each judgment rests on. It says which criteria it chose when you give none, and reports a criterion the transcript cannot settle as unassessable rather than guessing.
- **It can name a conversation, and so can you.** `rename_conversation` is the one tool in that group that writes, and all it writes is a title. In the sidebar a pencil on each row edits the name in place, and the editor it opens carries a ✨ that asks the model instead, reading the whole conversation and naming what it turned out to be about, which is worth more than the automatic title a conversation got from its opening message alone. The assistant's rename lands when its turn finishes rather than immediately, and its reply says so, because the write waits for the turn slot that turn is holding.
...
- **What the web front end adds on top.** The sidebar lists conversations, each titled by the model from the message that opened it (the truncated message shows immediately and the written title replaces it a moment later, or stands as it is if the model has nothing to say, or if you set `[assistant] generate_titles = false`), with new/delete, a duplicate control that copies a whole conversation into a new one titled "Copy of <the original>" without moving you off the one you are reading, and a collapsible, resizable rail whose state is remembered per browser. The list sits under a foldable Conversations title carrying the count of rows it shows, the same furniture the tasks section below it wears, and each section remembers its own fold. Each turn carries two controls on the message that opened it: a branch control that forks a new conversation holding everything through that turn and switches to it, so you can take one exchange two ways, leaving the original in the list untouched; and a delete-from-here control that removes that turn and everything after it, so a transcript is a record you can tidy rather than one that only grows, with the turns before it and the conversation itself left as they were. Below it a tasks section lists each task's schedule and next firing with pause/resume, run-now (Stop, while a run is under way), and delete on each row, and that task's own conversations nested under it, so the chat list above holds only conversations you started. It hides when you have no tasks and collapses when you do. Every row carries a localized datetime caption, revealed on hover, that survives reloads. Replies render as GitHub-flavored markdown *while they stream*, via vendored `marked` + `DOMPurify` (no CDN): the page reparses the whole accumulated answer on a 50ms timer rather than committing each finished block, because a newline is not a block boundary in CommonMark and a committed prefix could be wrong in a way no later token could repair. LaTeX (`$...$`, `$$...$$`) is typeset by vendored KaTeX *after* sanitization with `trust` disabled, and only once the turn completes, since half-typed TeX is noise. If the server goes away, from a restart or a dropped connection, the page reconnects on its own with backoff and resyncs, rather than sitting on a dead socket until you reload, and the notice it shows counts down to its next attempt with a Retry now button for when you know the server is already back; a turn that was in flight when the socket dropped comes back as whatever it persisted, not as a live stream, and a tab refused because another one holds the session says so and stays put instead of retrying. There is no settings window: the header's theme button is the one control the browser owns, cycling auto / light / dark and applied before first paint, and it is a per-browser preference rather than a `config.toml` setting. The CLI renders the same loop in plain text, one conversation on screen at a time, and reaches the others with `/new`, `/conversations`, and `/switch`.
...
`config.toml` is also **app-written**: the assistant's own `update_config` tool and a runtime `add_mcp_server` write back to it, with your comments preserved. There is no second settings store. Which keys `update_config` refuses is `[security] locked_config_keys`, a list of patterns that is yours to set; it ships locking seven patterns, the first being the whole `[security]` section rather than any one key in it: the approval gate's `confirm_tools`, the `[security.auto_approval]` sub-table and the `never_auto_approve` floor beneath it, and the lock list itself are all hand-edit-only by default. Then `[email] to`, `[paths] data_dir`, the whole `[agents.*]` section, and `[reviewers.*]` (a reviewer's prompt decides how the assistant's own work is judged), so which capability an agent holds stays your decision by default. `locked_config_keys` itself is always locked, whatever the list says, so the assistant can never unlock itself in one call. `[scheduling.task.*]` is refused too, but for a different reason that removing it from the list does not change: the assistant writes scheduled tasks through its own scheduling tools instead, because a bare `update_config` write would edit the file without arming or disarming the scheduler to match. `[compute] command_env_passthrough` is locked for a security reason of its own: without it the assistant could name one of its own credentials there and read it back out of a `run_command` child's environment.
...
**Tool approval.** The riskiest tools require confirmation before each call -- a `y/N` prompt in the terminal, Allow/Deny buttons in the web UI. By default this gates `skills.add_skill_script`, `mcp.add_mcp_server`, `compute.execute_python`, `compute.run_command`, `fs_write`, and `config.update_config`. Every entry names the toolset providing the tool, so a gate can name a whole capability (`fs_write` gates every file writer AIMU ships, including one added in a later release) as readily as one call into it (`compute.execute_python`, leaving `compute.calculate` alone), and the reserved `core` prefix holds the two tools no toolset provides, `spawn_subagent` and `activate_skill`. An entry that would gate nothing fails startup and says which of the four ways it does: a bare tool name with no prefix, the wrong toolset in front of a real tool, a toolset no agent declares, or a toolset that provides no tools. Adjust with `[security] confirm_tools` or `--confirm-tools compute.execute_python,config.update_config` (empty disables it). Gating is by tool name at call time, so it applies to a sub-agent's call as much as the assistant's own: a worker's gated call is routed to you. Proactive and backgrounded turns auto-deny these regardless, so the assistant never runs a full-access tool unattended, and an approval prompt only ever appears for the conversation you are currently viewing. A backgrounded turn's auto-deny raises an alert card naming the tool and linking the conversation, so a turn quietly losing a capability because you switched tabs is something you find out about; a proactive firing still denies in silence, since it reports itself when it ends.

**A reviewer can answer an approval prompt for you, and it ships off.** With `[security.auto_approval]` enabled, a gated call whose tool you named there is first shown to a declared model reviewer (`[reviewers.approval]` in `config.toml`, prompt and model included, so you can read and change the standard it is held to). It answers three questions and writes one sentence; Kokua computes the outcome from those answers, and both outcomes appear as a card. This is convenience, not a boundary: a review can only turn a prompt into an approval, never widen what you could have approved yourself, there is no deny verdict, and every way a review can fail lands on the ordinary prompt, except switching conversations during the review itself, which denies rather than prompting since nobody is left watching the turn the prompt would appear on. Three tools are floored in `[security].never_auto_approve` because they grant rather than act (`update_config`, `add_skill_script`, `add_mcp_server`), and an unattended turn is never auto-approved. There is no sandbox under any of this and no shell parsing in front of it, so read [Auto-approval](https://saxman.info/kokua/explanation/auto-approval/) before switching it on.

**Capability is granted by hand.** `[agents.*]` decides what every agent can call, and it is locked by default in `[security] locked_config_keys`: `update_config` refuses it. So the assistant can connect an MCP server, but it cannot give itself or a worker the server's tools: that takes an edit you make. Removing `agents.*` from `locked_config_keys` is a hand-edit that hands the assistant its own capability table, so leave it in place unless that is what you want. Note the flip side, since it is easy to misread: there is no privilege tier among agents. An agent whose table declares `config` really does get `update_config`, and one declaring `compute` really does get `execute_python` and `run_command`. Your hand-edit is the consent, and `confirm_tools` is the gate at call time. Which is also why reading a file and writing one are two toolsets: `fs` reads, `fs_write` writes, and they are separate names because with one name there would be no way to write down "read a file and do not write one". The shipped `introspector` declares `fs` to read a transcript it was asked to evaluate, and evaluating a conversation is no reason to be able to rewrite one.

**Injection crosses conversations.** The assistant can read every saved conversation, and a transcript is untrusted text (a worker may have pasted web content into it), so an injection that lands in one conversation can influence what the assistant does in another. The read tools are ungated by default, since they only read and gating them would make an unattended scheduled run that reads history fail silently; add `conversations.read_conversation` and `conversations.search_conversations` to `[security] confirm_tools` if you would rather approve each one (or a bare `conversations` for the whole capability). `rename_conversation` is ungated too, and it is the one that writes: the worst an injection gets from it is a conversation misleadingly named, since a title is the only field it can touch and no history is lost. Gate it as well if a sidebar you can trust at a glance matters to you. `export_conversation` is ungated as well, and its risk is a different one: it changes no conversation and can only write inside `data/downloads/` under a name taken from the conversation's own id, but what it leaves there is a whole transcript in the clear, in the folder the web UI serves unauthenticated. Gate it if that folder is what you are protecting.
